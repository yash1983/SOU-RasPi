#!/usr/bin/env python3
"""
SOU Raspberry Pi - Attraction C QR Scanner
Main application for Gate C ticket validation
"""
...
class AttractionCScanner:
    def __init__(self):
        """Initialize Attraction C scanner"""
        self.attraction_name = "AttractionC"
    ...
    def validate_ticket(self, qr_data):
        ...
        if not qr_data.startswith("TICKET_C") and not qr_data.startswith("TICKET_ABC"):
            return {
                'valid': False,
                'reason': 'Attraction mismatch - Ticket not valid for Attraction C',
                'ticket_info': None
            }
...
def main():
    """Main function"""
    scanner = AttractionCScanner()
    success = scanner.run()
    
    if success:
        print("✅ Attraction C scanner completed successfully!")
        return 0
    else:
        print("❌ Attraction C scanner failed")
        return 1
